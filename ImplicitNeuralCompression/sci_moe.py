...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", type=str, default='0,5,6,7', help="gpu index")
    ...
    # hist = cv2.calcHist([data], [0], None, [65536], [0, 65536])
    ...
    data_shape = ",".join([str(i) for i in data.shape]) # 64,64,64,1
    ...
    if sampling_required:
        ...
        coords_batch = rearrange(coordinates, "d h w c-> (d h w) c").cuda()
        gt_batch = rearrange(normalized_data, "d h w c-> (d h w) c").cuda()
    if sampling_required:
    ...
    # initialize network
    network = MoE(coords_channel=3, data_channel=1, num_experts=2, features=n_network_features // 2 , noisy_gating=True, k=1)
    ...
    #size_by_stattorch = stat(network,(64,64,64,1))
    #print(size_by_stattorch)
    ...
    alpha = 0.0
    beta = 0.0
    # test_coord,_ = sampler.next()
    for steps in range(1, n_training_steps + 1):
        if sampling_required:
            coords_batch, gt_batch = sampler.next()
            coords_batch, gt_batch = coords_batch.to('cuda'), gt_batch.to('cuda')
        optimizer.zero_grad()
        pred, aux_loss = network(coords_batch)
        torch.autograd.set_detect_anomaly(True)
        loss = l2_loss(gt_batch, pred) + aux_loss.mean()
        loss.backward()
        optimizer.step()
        lr_scheduler.step()
        ...
        if steps % n_print_loss_interval == 0:
            ...
            # feature = network.first_linear(test_coord)
            # softmax_out = network.gate(feature)
            # expert_outputs = network.test(test_coord)
            # print("测试坐标Gating输出")
        ...
        if steps in checkpoints:
            ...
            with torch.no_grad():
                ...
                for batch_idx in range(n_inference_batchs):
                    ...
                    ],_ = network(flattened_coords[start_sample_idx:end_sample_idx].cuda())
