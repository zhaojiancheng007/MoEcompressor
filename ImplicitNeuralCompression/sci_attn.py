...
from utils import *
from split import PatchEmbed, split, Mydataset
...
class MoE(nn.Module):
    ...
    def forward(self, coords):
        ...
        # output = torch.masked_select(output, indices_to_remove)
        # output = torch.mul(index, output)
        # output = output.reshape([-1,self.topk])
...
##########开始训练#####################################################################
    #zjc
    coords_split = PatchEmbed(coordinates, pool_size=4, pool_stride=4, block_size=8, block_stride=4)
    coords = coords_split(coordinates)
    gt_split = PatchEmbed(normalized_data, pool_size=4, pool_stride=4, block_size=8, block_stride=4)
    gt = gt_split(normalized_data)
    weights_split = PatchEmbed(weight_map, pool_size=4, pool_stride=4, block_size=8, block_stride=4)
    weights = weights_split(weight_map)
    
    #获取datasets类
    coords = Mydataset(coords)
    gt = Mydataset(gt)
    weights = Mydataset(weights)
    
    coords_loader = DataLoader(coords, batch_size=128, shuffle=False)
    gt_loader = DataLoader(gt, batch_size=128, shuffle=False)
    weights_loader = DataLoader(weights, batch_size=128, shuffle=False)
    ...
    for steps in range(1, n_training_steps + 1):
        ...
        loops = 0.
        optimizer.zero_grad()
        for coords_batch, gt_batch, weight_map_batch in zip(coords_loader, gt_loader, weights_loader): 
            optimizer.zero_grad()
            predicted_batch = network(coords_batch)
            loss = l2_loss_attn(gt_batch, predicted_batch, weight_map_batch)
        ...
        if steps % n_print_loss_interval == 0:
            ...
            print(
                f"#Steps:{steps} Loss:{running_loss // loops} ElapsedTime:{compression_time_seconds}s",
                flush=True,
            )
