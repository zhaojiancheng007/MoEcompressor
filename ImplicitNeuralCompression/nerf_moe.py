...
class MoE(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.first_linear = nn.Sequential(nn.Linear(in_channel, gating_feature))
        ...
        for i in range(experts_num):
            ...
            self.expert.append(nn.Sequential( \
                NeRF_zcq(features=n_network_features//experts_num, coords_channel = gating_feature, \
                layers = expert_layers)))
                #  (coords_channel = gating_feature, layers=expert_layers, \
    ...
    def forward(self, coords):
        ...
        feature = codings
        feature = self.first_linear(feature)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c",
        type=str,
        default=opj(opd(__file__), "config", "SingleExp", "nerf_3d_zcq.yaml"),
        help="yaml file path",
    )
    parser.add_argument("-g", type=str, default="4", help="gpu index")
